...
def default_config(data='AIR_BJ'):
    ...
    gpu_id = GPU().get_usefuel_gpu(max_memory=6000, condidate_gpu_id=[0,1])
...
def main(params: dict):
    ...
    if (params['trial_name'] == ""):
        ...
        config.trial_name = params['trial_name']
    ...
    for epoch in range(config.epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            time_lst.append((timer() - time_start))
            message = f"{i / len(train_loader) + epoch:5.1f} | {avg_loss:0.3f} {np.sum(time_lst):.1f}s"
